# SETUP INICIAL - rodar UMA VEZ so, depois de:
...
#  nao dentro de uma subpasta a mais - se o zip criar uma pasta duplicada
#  tipo tools\node-v24.21.0-win-x64\node-v24.21.0-win-x64\, mova um nivel
#  pra cima antes de continuar)
#
# O que este script faz:
#   1. Libera o "import site" no Python embutido (vem desativado por
#      padrao, e sem isso o pip nao funciona nem depois de instalado)
#   2. Instala o pip nesse Python, via get-pip.py
#   3. Instala as dependencias do backend (requirements.txt)
#   4. Cria o backend\.env a partir do .env.example (se ainda nao existir)
#   5. Instala as dependencias do frontend (npm install)
#
# Se der erro de politica de execucao ao tentar rodar este arquivo:
#   Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
# E se isso tambem for bloqueado (comum em laboratorio com politica de
# dominio), os passos abaixo podem ser feitos manualmente no cmd.exe -
# veja o comentario de cada etapa.
...
if (-not (Test-Path (Join-Path $nodeDir "node.exe"))) {
    Fail "Nao achei $nodeDir\node.exe - extraia o zip do Node dentro de tools\ (veja o comentario no topo deste arquivo)."
}
if (-not (Test-Path (Join-Path $pyDir "python.exe"))) {
    Fail "Nao achei $pyDir\python.exe - extraia o zip do Python dentro de tools\ (veja o comentario no topo deste arquivo)."
}
...
if (-not $pthFile) {
    Fail "Nao achei o arquivo *._pth dentro de $pyDir - confere se o zip do Python foi extraido certo."
}
...
if ($pthContent -match "^\s*#\s*import site\s*$") {
    ...
    Write-Host "  'import site' ja estava liberado, ok." -ForegroundColor Green
} else {
    Write-Host "  Aviso: nao achei a linha '#import site' em $($pthFile.Name) - confere manualmente." -ForegroundColor Yellow
}
...
if (-not $hasPip) {
    ...
    Write-Host "  pip ja estava instalado, ok." -ForegroundColor Green
}

Write-Host "== 4/5: instalando dependencias do backend ==" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { Fail "Falha ao instalar as dependencias do backend (pip)." }
...
if (-not (Test-Path $envFile)) {
    ...
    Write-Host "  backend\.env ja existe, nao mexi nele." -ForegroundColor Green
}

Write-Host "== 5/5: instalando dependencias do frontend (pode demorar) ==" -ForegroundColor Cyan
# equivalente manual: cd frontend ; npm install
Push-Location (Join-Path $root "frontend")
npm install
$npmExit = $LASTEXITCODE
Pop-Location
if ($npmExit -ne 0) { Fail "Falha ao instalar as dependencias do frontend (npm install)." }
